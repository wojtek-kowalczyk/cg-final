...
std::pair<std::shared_ptr<Mesh>, std::shared_ptr<Material>> processMesh(aiMesh* mesh, const aiScene* scene)
{
    ...
    auto myMesh = std::make_shared<Mesh>((float*)vertices.data(), vertices.size(), indices.data(), indices.size(), layout);
    auto myMaterial = std::make_shared<Material>(glm::vec3(1.0f, 1.0f, 1.0f), Shaders::basicLit(), diffuseMaps, specularMaps, 128.0f);

    return std::make_pair(myMesh, myMaterial);
}

std::vector<std::pair<std::shared_ptr<Mesh>, std::shared_ptr<Material>>> processNode(aiNode* node, const aiScene* scene)
{
    std::vector<std::pair<std::shared_ptr<Mesh>, std::shared_ptr<Material>>> meshesAndMaterials;
